...
SENSITIVE_PATTERNS = {
    "CRITICAL (高危 - 隐私/凭证泄露)": {
        ...
        "patterns": [
            ...
            # 新增：加密/编码外发特征 (Atlas-01 案例)
            r"base64\.b64encode", r"btoa\(", r"crypto", r"AES", r"RSA", r"hashlib", r"zlib", r"gzip"
        ]
    },
    "WARNING (中危 - 环境访问/外部通信)": {
        ...
        "patterns": [
            ...
            r"axios\.post", r"requests\.post", r"webhook", r"socket", r"urllib\.request"
        ]
    },
    "INFO (低危 - 脚本执行)": {
        "level": "🔵 LOW",
...
def scan_file(file_path):
    ...
    try:
        ...
        findings.append({"category": "Error", "level": "UNKNOWN", "pattern": str(e)})
    return findings
...
def main():
    ...
    for root, dirs, files in os.walk(skill_path):
        ...
        for file in files:
            ...
            if findings:
                ...
                print(f"\n📂 文件: {rel_path}")
                for f in findings:
                    print(f"  [{f['level']}] {f['category']}: 发现 '{f['pattern']}' (第 {f['line']} 行)")
                    print(f"    └─ 代码: {f['content']}")
    ...
    if risk_summary['🔴 HIGH'] > 0:
        print("\n❌ 结论: 发现高危隐私访问或异常加密行为！严禁执行安装。")
        sys.exit(1)
    elif risk_summary['🟡 MEDIUM'] > 0:
        print("\n⚠️ 结论: 存在敏感操作（联网/环境），请人工核对逻辑。")
    else:
        print("\n✅ 结论: 扫描通过，未发现明显的隐私风险。")
